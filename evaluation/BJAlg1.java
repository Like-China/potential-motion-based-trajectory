...
public class BJAlg1 {
    ...
    public BJAlg1(int tsNB, double repartirionRatio, int minLeafNB) {
        this.tsNB = tsNB;
     ...
     */
    public ArrayList<TimeIntervalMR> getIntersectionAtOneTime(int ts, TimeIntervalMR q) {
        TernaryBallTree bt = ballTreeAtEachTimestamp.get(ts);
        TernaryBallNode root = ballRootAtEachTimestamp.get(ts);
        long t1 = System.currentTimeMillis();
    ...
    public HashMap<Integer, ArrayList<TimeIntervalMR>> getIntersectionAtAllTime(Trajectory trj) {
        HashMap<Integer, ArrayList<TimeIntervalMR>> ts2candidate = new HashMap<>();
        for (int ts = 0; ts < tsNB - 1; ts++) {
            ArrayList<TimeIntervalMR> intersect = this.getIntersectionAtOneTime(ts, trj.EllipseSeq.get(ts));
            ts2candidate.put(ts, intersect);
        }
        return ts2candidate;
    }

    public int getAll(ArrayList<Trajectory> queries, ArrayList<Trajectory> db, double simThreshold) {
        long t1 = System.currentTimeMillis();
        int matchNB = 0;
        int i = 0;
        for (Trajectory qTrj : queries) {
            ...
            for (Trajectory dbTrj : db) {
                double simUpper = qTrj.sampleLocsimTo(dbTrj);
                simUpper *= Settings.tsNB;
